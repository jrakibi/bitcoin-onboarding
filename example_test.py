...
class ExampleTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Extend the main chain by having node 2 create block with height 12. 
        block = int(self.generate(self.nodes[2], sync_fun=lambda: self.sync_all(self.nodes[0:3]), nblocks=1) [0], 16)
        
        # Attach pdb for debugging
        # import pdb; pdb.set_trace()
        
        # What are different ways you can test that node 1 received the block?
        # Method 1
        assert_equal(12, self.nodes[1].getblockcount())
        # Method 2
        bestBlockHashNode1 = self.nodes[1].getblockhash(12)
        bestBlockHashNode2 = self.nodes[2].getblockhash(12)
        assert_equal(bestBlockHashNode1, bestBlockHashNode2)
